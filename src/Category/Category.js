...
function Load(){
  $(document).ready(function(){
    ...
    createCookie('currentpage','category',null,30);
...
function Save(){
  ...
  postTransactionEmpty(settings.endpoints.category.POST,userentry
    ,LoadCategory,'Category was recorded successfully!');
...
function Delete(id){
  ...
  postTransactionEmpty(settings.endpoints.category.PUT,userentry
    ,LoadCategory,'Category was updated successfully!');
...
function LoadCategory(filter){
  getTransactionFilter(settings.endpoints.category.GET,LoadPageDetail,filter);
}
...
function Category() {
  ...
  return (
    <div>
        <BreadCrumb local={<span>Projetos</span>} class="left" /><span class="left" >-- Categorias</span>
        <br/>
        <h3 data-key="CATEGORY" class="keycontainer" >Category</h3>
        <div class="row" >
          <div class="col-sm-4" >
